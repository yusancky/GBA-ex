...
const 保存文件名 = `[gba-ex.netlify.app]大湾区制霸.png`;
...
添加事件监控(地区, 点击, (事件) => {
  ...
  左 = 数学[最小](左, 体元素.offsetWidth - 设置等级元素方位[宽度] - 最小间距);
  左 = 数学[最大](左, 最小间距);
  ...
  上 = 数学[最小](上, 体元素.offsetHeight - 设置等级元素方位[高度] - 最小间距);
  上 = 数学[最大](上, 最小间距);
...
const 获取字体数据地址 = (地址, 回调) => {
  取(地址)
    [那么]((资源) => 资源.blob())
    [那么]((原始数据) => 读文件成地址(原始数据, 回调));
};
const 获取字体样式 = (字体名, 回调) => {
  获取字体数据地址(`${字体名}.woff?v={version}`, (地址) =>
    回调(`@font-face{font-family:${字体名};${源}:url(${地址})}`)
  );
};
获取字体样式(字体名, (样式字串) => {
  $(样式, 图形)[肉] = 样式字串;
...
const 日志 = (_) =>
  (新建图()[
    源
  ] = `https://lab.magiconch.com/api/china-ex/log?levels=${获取所有省等级们字符串()}&r=${
    文档[来源]
  }`);
